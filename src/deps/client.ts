...
import { PackageVersionsResponse, LatestVersionResponse, DepsError } from '../types/index.js';
import { valid, rcompare, parse as parseSemver } from 'semver';

const DEPS_API_BASE = 'https://api.deps.dev/v3alpha';
const MAX_RETRIES = 3;
...
/**
 * DepsClient - Client for deps.dev API v3alpha
 */
export class DepsClient {
  ...
  async getPackageVersions(system: SystemType, name: string): Promise<PackageVersionsResponse> {
    ...
    const url = `${DEPS_API_BASE}/systems/${system}/packages/${encodedName}`;
